...
mu_eff      = sqrt(mu_a/D);
l_t_prime   = 1/(mu_a+mu_s_prime); % Transport mean free path
    ...
    while(1)
        ...
        if(d_b*mu_t <= photon(1,9)) % Hits boundary? Use 3.33 to see if it will hit the boundary
            ...
            if rand <= R_i
                ...
                [~,i_r] = min(abs(sqrt(photon(1,1)^2+photon(1,2)^2+photon(1,3)^2)-r));
                R_r_temp(i_r,1) = R_r_temp(i_r,1) + photon(1,7);
            ...
            [~,i_r] = min(abs(sqrt(photon(1,1)^2+photon(1,2)^2+photon(1,3)^2)-r));
            A_r_temp(i_r,1) = A_r_temp(i_r,1) + photon(1,7)*(mu_a/mu_t);
...
semilogy(r,F_r,'.');
hold on;
semilogy(r,exp(-mu_eff*r)./(4*pi*D*r));
